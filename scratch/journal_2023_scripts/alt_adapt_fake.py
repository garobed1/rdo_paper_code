...
trueFunc = prob#GetProblem(prob, dim)#, use_design=ud)
...
for m in range(iters):
    xtrain = sampling(nt0+m)
    ftrain = trueFunc(xtrain)
    gtrain = np.zeros_like(xtrain)
    for j in range(dim):
        gtrain[:,j:j+1] = trueFunc(xtrain, kx=j)
    ...
    for j in range(len(models)):
        ...
        if(ftrain.shape[0] > 0):
            ...
            plt.savefig(f"{title}/2d_FAKE_errcon_a.pdf", bbox_inches="tight")
            plt.clf()
            import pdb; pdb.set_trace()


        errors[j].append(rmse(models[j], trueFunc, N=10000))
...
if rank == 0:
    ...
    with open(f'{title}/FAKEsamplehist.pickle', 'wb') as f:
        pickle.dump(samplehist, f)

    with open(f'{title}/FAKEerrors.pickle', 'wb') as f:
