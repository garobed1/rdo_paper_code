...
"""
Alternative to ens plots but for results generated the alt_models.py way
"""
...
# the four targets
altdatanames = ['errhrmsgek', 'errkrmsgek', 'errhrmskrg', 'errkrmskrg']
altdatamodel = [GEKPLS(), GEKPLS(), KRG(), KRG()]
...
for i in range(len(titles)):
    ...
    if isinstance(modelf[-1], PCEStrictSurrogate):
        ...
        ehr.append([])
        ehr[-1].append(errhrms[-1])
        ekr.append(None)
        for j in range(nprocs[-1]):
            xk[-1].append(None)
            fk[-1].append(None)
            gk[-1].append(None)
            mf[-1].append(modelf[-1])
            hi[-1].append([hist[-1][k] for k in range(len(hist[-1]))])
        ...
        nprocs.append(len(modelf[-1]))
        mname.append(modelf[-1][0][0].name)
        ...
        e0rc = np.atleast_2d(np.array(err0rms[-1]).squeeze()).T
        ehrc = np.array(errhrms[-1]).squeeze()
        ehr.append(np.append(e0rc, ehrc, axis = 1))
        ekr.append(np.array(errkrms[-1]).squeeze())
        for j in range(nprocs[-1]):
            xk[-1].append(xtrainK[-1][j][:])
            fk[-1].append(ftrainK[-1][j][:])
            gk[-1].append(gtrainK[-1][j][:])
            # ekr[-1].append( errkrms[-1][j][:])
            mf[-1].append(modelf[-1][j][:])
            # e0r[-1].append(err0rms[-1][j])
            hi[-1].append(hist[-1][j][:])
            # ehr[i].append(errhrms[i][j][:])


        # NOW TRY ADDING THE ALT DATA
        for k in range(2):
            try:
                with open(f'{titles[i]}/{altdatanames[2*k]}.pickle', 'rb') as f:
                    errhrms.append(pickle.load(f))
                with open(f'{titles[i]}/samplehist.pickle', 'rb') as f:
                    hist.append(pickle.load(f))
                modelf.append(altdatamodel[2*k])
                xtrainK.append(None)
                ftrainK.append(None)
                gtrainK.append(None)
                try:
                    with open(f'{titles[i]}/{altdatanames[2*k+1]}.pickle', 'rb') as f:
                        errkrms.append(pickle.load(f))
                except:
                    errkrms.append(None)

                nprocs.append(len(modelf[0]))
                mname.append(modelf[-1].name)
                adapt.append(titles[i].split("_")[2]) #should hit either hess or sfcv
                xk.append([]) 
                fk.append([]) 
                ...
                e0rc = np.atleast_2d(np.array(err0rms[-1]).squeeze()).T
                ehrc = np.array(errhrms[-1]).squeeze()
                ehr.append(np.array(errhrms[-1]).squeeze())
                ekr.append(np.array(errkrms[-1]).squeeze())
                for j in range(nprocs[-1]):
                    xk[-1].append(None)
                    fk[-1].append(None)
...
try:
    with open(f'{titles[i]}/samplehistk.pickle', 'rb') as f:
...
if nmodels > 1:
    ...
    for i in range(nmodels):
        if not isinstance(modelf[i], PCEStrictSurrogate):
            nmax = min(nmax, sh[i][-2]*facdict[mname[i]])
    for i in range(nmodels):
        if not isinstance(modelf[i], PCEStrictSurrogate):
            ...
            try:
                ...
                try:
                    ...
                    imax[i] = np.argmin(abs(nmax-sh[i]*int(facdict[mname[i]])))     
        try:
